<?php

    $info = get_field('info');
    $enhanced_copy = $info['enhanced_copy'] ?? '';
    $original_copy = $info['original_copy'] ?? '';

    $photos = get_field('photos');
    $enhanced = $photos['enhanced'] ?? '';
    $high_resolution_file = $photos['high_resolution_file'] ?? '';

    if($high_resolution_file) {
        $high_resolution_url = $high_resolution_file['url'] ?? '';
    } else {
        $high_resolution_url = $enhanced['url'] ?? '';
    }

?>

<div class="photo__info">
    <div class="photo__info-copy">
        <div id="original-copy" class="photo__info-copy-original | copy copy-2 flow active">
            <?php if ($original_copy): ?>
                <?php echo $original_copy; ?>
            <?php endif; ?>
        </div>

        <div id="enhanced-copy" class="photo__info-copy-enhanced | copy copy-2 flow">
            <?php if ($enhanced_copy): ?>
                <?php echo $enhanced_copy; ?>
            <?php endif; ?>
        </div>
    </div>

    <div class="photo__info-utilities">
        <!-- High Resolution Download Link (default hidden, controlled by JS) -->
        <a href="<?php echo $high_resolution_url; ?>" target="window" id="high-res-download" class="photo__info-utilities-link | download" style="display:none;" download>
            <svg width="17" height="17" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" clip-rule="evenodd" d="M12.57 7.28886C12.4093 7.12834 12.1914 7.03818 11.9643 7.03818C11.7371 7.03818 11.5193 7.12834 11.3586 7.28886L9.10714 9.54029V1.10714C9.10714 0.879814 9.01684 0.661797 8.85609 0.501051C8.69535 0.340306 8.47733 0.25 8.25 0.25C8.02267 0.25 7.80465 0.340306 7.64391 0.501051C7.48316 0.661797 7.39286 0.879814 7.39286 1.10714V9.54029L5.14143 7.28886C4.97894 7.13745 4.76403 7.05503 4.54197 7.05894C4.31992 7.06286 4.10805 7.15282 3.951 7.30986C3.79396 7.4669 3.704 7.67877 3.70009 7.90083C3.69617 8.12289 3.77859 8.3378 3.93 8.50029L7.64429 12.2146L8.25 12.8214L8.85571 12.2157L12.57 8.50143C12.6497 8.42183 12.7128 8.32731 12.7559 8.22328C12.7991 8.11926 12.8212 8.00775 12.8212 7.89514C12.8212 7.78254 12.7991 7.67103 12.7559 7.567C12.7128 7.46297 12.6497 7.36846 12.57 7.28886ZM1.96429 10.25C1.96429 10.0227 1.87398 9.80465 1.71323 9.64391C1.55249 9.48316 1.33447 9.39286 1.10714 9.39286C0.879814 9.39286 0.661797 9.48316 0.501051 9.64391C0.340306 9.80465 0.25 10.0227 0.25 10.25V13.9643C0.25 14.5705 0.490816 15.1519 0.91947 15.5805C1.34812 16.0092 1.92951 16.25 2.53571 16.25H13.9643C14.5705 16.25 15.1519 16.0092 15.5805 15.5805C16.0092 15.1519 16.25 14.5705 16.25 13.9643V10.25C16.25 10.0227 16.1597 9.80465 15.9989 9.64391C15.8382 9.48316 15.6202 9.39286 15.3929 9.39286C15.1655 9.39286 14.9475 9.48316 14.7868 9.64391C14.626 9.80465 14.5357 10.0227 14.5357 10.25V13.9643C14.5357 14.1158 14.4755 14.2612 14.3683 14.3683C14.2612 14.4755 14.1158 14.5357 13.9643 14.5357H2.53571C2.38416 14.5357 2.23882 14.4755 2.13165 14.3683C2.02449 14.2612 1.96429 14.1158 1.96429 13.9643V10.25Z" fill="#8ACAD7"/>
            </svg>
            Download Full Hi-Res
        </a>

        <a href="<?php echo site_url('/photo-gallery/'); ?>" class="photo__info-utilities-link | back">
            <svg width="16" height="13" viewBox="0 0 16 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M0.265313 5.38056C0.0954333 5.55049 0 5.78094 0 6.02122C0 6.2615 0.0954333 6.49195 0.265313 6.66188L5.3915 11.7881C5.56241 11.9531 5.79131 12.0445 6.0289 12.0424C6.26649 12.0403 6.49377 11.945 6.66178 11.777C6.82979 11.609 6.92509 11.3817 6.92716 11.1441C6.92922 10.9066 6.83789 10.6777 6.67282 10.5067L3.09346 6.92739H15.0938C15.3342 6.92739 15.5647 6.83192 15.7346 6.66198C15.9045 6.49204 16 6.26155 16 6.02122C16 5.78089 15.9045 5.5504 15.7346 5.38046C15.5647 5.21052 15.3342 5.11505 15.0938 5.11505H3.09346L6.67282 1.53569C6.83789 1.36479 6.92922 1.13589 6.92716 0.898293C6.92509 0.660699 6.82979 0.433421 6.66178 0.26541C6.49377 0.0973993 6.26649 0.00209888 6.0289 3.42555e-05C5.79131 -0.00203037 5.56241 0.0893061 5.3915 0.254372L0.265313 5.38056Z" fill="#8ACAD7"/>
            </svg>
            Back to Main Gallery
        </a>
    </div>
</div>